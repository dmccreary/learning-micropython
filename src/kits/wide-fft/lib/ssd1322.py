...
set set_orientation by Dan McCreary in March of 2025.
"""
__version__ = '0.2'
...
class SSD1322:
    ...
    def init_display(self):
        ...
        # original value was 0x14
        # the new value was 0xA0
    ...
    def rotate(self, rotate):
        if rotate:
            # Use the working configuration (Setting 3)
            self.write_cmd(0xA0)
            self.write_data(0x04)  # v_flip=True, h_flip=False, nibble_remap=False
            self.write_data(0x11)  # Keep Dual COM mode
        else:
            # Original orientation
            self.write_cmd(0xA0)
            self.write_data(0x14)  # Original value from init
            self.write_data(0x11)  # Keep Dual COM mode    

    def set_orientation(self, h_flip=False, v_flip=True, nibble_remap=False):
            remap_byte = 0x00
            if h_flip:
                remap_byte |= 0x01  # Bit 0: Column Address Remap
            if v_flip:
                remap_byte |= 0x04  # Bit 2: COM Remap
            if nibble_remap:
                remap_byte |= 0x10  # Bit 4: Nibble Remap
                
            self.write_cmd(0xA0)
            self.write_data(remap_byte)
            self.write_data(0x11)  # Keep Dual COM mode
    ...
    def show(self):
        ...
        col_start=offset//4
        # Adjust column addressing to fix the 16-pixel shift
        col_start += 4  # Shift right by 16 pixels (4 nibbles)
