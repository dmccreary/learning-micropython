...
def init_display():
    ...
    # Turn on display with medium brightness (0x41 to 0x7F)
    for addr in DISPLAY_ADDR:
        i2c.writeto(addr, b'\x00')  # Clear display
    sleep(0.1)
    # Re-enable with brightness setting
    for addr in DISPLAY_ADDR:
...
def display_value(value):
    ...
    # For a 4-digit display showing 0-100, we'll use the rightmost 3 digits
    
    # First digit (leftmost) - blank if value < 100
    if value < 100:
        digit_pattern = SEGMENT_PATTERNS[' ']
    else:
        digit_pattern = SEGMENT_PATTERNS[1]  # Must be 1 for 100
    i2c.writeto(DISPLAY_ADDR[0], bytes([digit_pattern]))
    
    # Second digit - blank if value < 10, otherwise tens digit
    if value < 10:
        digit_pattern = SEGMENT_PATTERNS[' ']
    else:
    ...
    # Third digit - always shows ones place
    digit_pattern = SEGMENT_PATTERNS[value % 10]
    i2c.writeto(DISPLAY_ADDR[2], bytes([digit_pattern]))
    
    # Fourth digit (rightmost) - blank (or could show % symbol if available)
    i2c.writeto(DISPLAY_ADDR[3], b'\x00')
