...
def init_display():
    """Initialize the display like the test script that works"""
    # Reset all display addresses
    for addr in [0x34, 0x35, 0x36, 0x37]:
        i2c.writeto(addr, b'\x00')
    sleep(0.1)
    
    # Enable main control register with a basic control value
    i2c.writeto(0x34, b'\x01')  # Enable display with minimal settings
    sleep(0.1)
