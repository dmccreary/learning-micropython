...
TWIDDLE_FACTORS = [math.exp(-2j * math.pi * k / SAMPLES) for k in range(SAMPLES//2)]

def compute_fft(samples):
    """Compute FFT using Cooley-Tukey algorithm with pre-computed twiddle factors"""
    n = len(samples)
    if n <= 1:
        return samples
    ...
    T = [0] * n
    for k in range(n//2):
        t = odd[k] * TWIDDLE_FACTORS[k * SAMPLES//n]  # Scale index for the current FFT size
        T[k] = even[k] + t
        T[k + n//2] = even[k] - t
