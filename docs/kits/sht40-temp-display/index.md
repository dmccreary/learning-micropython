...
![SHT40 Temp Sensor with SmartWatch Display Kit](./sht40-smartwatch-display.jpg)
...
*This is a great way to see a highly responsive and precise sensor that measures temperature and humidity.  The image
above was generated by a screen simulator*
...
7. Add a push button and use it to switch the watch between modes.
...
| `main.py` | A copy of program 09, the smartwatch. The Pico runs it by itself when it starts up |
...
5. Watch the smartwatch start on its own!

`main.py` is a copy of program 09. It starts on the Watch face you built. In
...
So far your watch has one screen. Now you will add a **push button**. Each
tap switches the watch to a new **mode**. A mode is a different way to show
the air.
...
![Six round watch screens in a grid. Top row: the classic watch face with 72.5 degrees F and 48.2 percent humidity; a green smiling face with round glasses; and a line graph of temperature with a thin cursor line. Bottom row: two rainbow ring gauges around a temperature and humidity number; a big thermometer filling with orange as it climbs toward a 90 degree goal; and a records screen showing a high of 78.4 and a low of 66.1 degrees F.](./smartwatch-modes.png){ width="720" }
...
| Mode | What you see |
|------|--------------|
| Watch | The watch face from Step 6 |
...
A mood ring changes color with your mood. Your watch changes color with the
...
| Mode | One of the ways the watch shows the air, such as Buddy or Live |
...
Step 7 (the standalone watch), Step 10 (change a setting), Step 11 (the button
...
| `09-smartwatch-modes.py` | The six-mode smartwatch |
