# Build a Temperature and Humidity Smartwatch

![Round smartwatch screen with a white ring around the edge. At the top is the word TEMPERATURE. Below it, 72.5 degrees F in big green letters, and 22.5 C in small white letters. A thin grey line splits the screen. Under the line is the word HUMIDITY, then 48.2% in big cyan letters, and a bar that is a little less than half full.](./smartwatch-face.png){ width="360" }

*This is the watch face you will build. A computer program that pretends to
be the screen drew this picture, so your real screen may look a little
different.*

!!! mascot-welcome "Welcome to the Smartwatch Lesson"
    ![Monty waving welcome](../../img/mascot/welcome.png){ class="mascot-admonition-img" }
    Today you will build a tiny round smartwatch that shows how warm the air
    is and how damp it is. You will wire it, test it, and read its code one
...
6. Change a setting and predict what the watch face will do.
...
| Time needed | About 90 minutes. Two 45-minute class periods works well |
| Difficulty | Beginner |
...
| New ideas in this lesson | Screens made of dots, x and y positions, percent, and choosing with `if` |
...
| SHT40 sensor | A **sensor** is a part that measures the world. This one measures temperature and humidity | Skin that feels the air |
| GC9A01 display | A round color screen, 240 dots wide and 240 dots tall | The face of the watch |
...
your watch is **relative humidity**. It compares the water in the air to the
...
| GC9A01 round display | 240 x 240 dots, about the size of a watch face. It has 7 or 8 pins |
| Solderless breadboard | Half-size is plenty |
| About 11 jumper wires | 4 for the sensor and 7 for the display. See the note below |
...
| `01` to `07` | The lab programs you will run in this lesson |
| `main.py` | A copy of the watch program. The Pico runs it by itself when it starts up |

When it finishes, it says `Uploaded 13 files` and lists what is on the Pico.
...
ready for the watch face. If the screen is dark, check the
[Troubleshooting](#troubleshooting) table.

## Step 6: Run the Watch Face

Open `07-display-temp-humidity.py` and click **Run**. Look at the picture at
the top of this page. Your watch should look like it!

Here is what each part of the watch face means:
...
![Four round watch faces in a two by two grid. Top left shows 60.1 degrees F in cyan with a small humidity bar. Top right shows 72.5 degrees F in green. Bottom left shows 82.0 degrees F in orange. Bottom right shows 95.0 degrees F in red with a nearly full humidity bar.](./temperature-colors.png){ width="480" }
...
sensor? Then your watch works. Well done!

## Step 7: Make It a Real Watch
...
5. Watch the watch face start on its own!
...
| Move the watch from a sunny spot to a shady spot | | | |
...
The full watch program is 273 lines long. Do not worry! We will look at four
small pieces. Each piece teaches one idea.

### Piece 1: Celsius to Fahrenheit

The sensor speaks Celsius. Your watch also shows Fahrenheit. This line does
the change:
...
The watch draws a new temperature every second. Look at what `%5.1f` does:
...
| The watch starts by itself and Thonny will not connect | That is `main.py` running. Click Thonny's red **Stop** button |
...
![Round watch face where the top word is the red text SENSOR FAIL instead of TEMPERATURE. The last good numbers, 72.5 degrees F and 48.2 percent, are still shown below](./sensor-fail.png){ width="240" }
...
??? question "2. Why did we test the sensor before wiring the display? (Click to reveal the answer)"
    ...
    watch face fails later, we know the trouble is probably in the display.
...
!!! mascot-celebration "Great Work, Maker!"
    ...
    You built a real smartwatch that feels the air and shows what it finds!
...
| 2 | 10 min | Run the watch face (Step 6, Checkpoint 3) |
| 2 | 15 min | Science Time (Step 8) and the science log |
| 2 | 10 min | Read the code together (Step 9) and check understanding |

Step 7 (the standalone watch), Step 10 (change a setting) and the Challenges
do not fit in the two periods. Use them for early finishers, a third
period, or homework.
...
- **What humidity means.** Students often think 50 percent means the air is
  ...
  temperature.
- **`main.py` seems to trap the Pico.** The watch runs by itself, so Thonny
...
you fix one thing at a time. The six quiz questions above also make a good
exit ticket.
...
| `lib/` | The screen's helper code: a drawing library, two fonts and a shapes file |
...
| `07-display-temp-humidity.py` | The smartwatch face |
| `upload-code.sh` | Copies everything to the Pico. It also saves lab 07 as `main.py` |
